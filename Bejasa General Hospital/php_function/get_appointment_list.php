...
$query="SELECT * FROM tbl_appointment WHERE Day='$day' AND Doctor='$id' AND Accepted='ACCEPTED' AND `Date` >= '$date_today'";
	...
	while($var = mysqli_fetch_array($result)){
        ...
        $patient_id = $var["Patient"];
                    ...
                    if(mysqli_num_rows($result1) > 0 ) {
                      ...
                      $output.='<div class="col-md-3"><div class="card mb-4  img-size bg-loob"><img  class="card-img-top" src="data:image/jpeg;base64,'.base64_encode( $var1["Photo"] ).'" alt="Card image cap" height="300"><center style="color:white">'.$var1["FirstName"].' '.$var1["MiddleName"].' '.$var1["LastName"].'<br>'.$var["Date"].'</center></div></div>';
