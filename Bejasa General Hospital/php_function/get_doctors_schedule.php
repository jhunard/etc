...
$mindate = date("Y-m-d", strtotime('today'));

$setdatetime='<img src="icon/date.png"> Select Date<br><input type="date" class="form-control" max="'.$maxdate.'" min="'.$mindate.'" onkeydown="return false" id="txt_date"> <center><i>*** Selected date must be within this month ***</i></center>';
...
if(mysqli_num_rows($result1) > 0 ) {
	while($var = mysqli_fetch_array($result1)){
		...
		array_push($response,array("name"=>'Dr. '.$var["FirstName"].' '.$var["MiddleName"].' '.$var["LastName"],"datetime"=>$setdatetime,"schedule"=>getMonday($id).getTuesday($id).getWednesday($id).getThursday($id).getFriday($id).getSaturday($id).getSunday($id)));
